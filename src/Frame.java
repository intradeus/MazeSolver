...
public class Frame extends JPanel
	...
	public static ArrayList<Node> coordImage;
	...
	public Frame() throws IOException {
		...
		window.getContentPane().setPreferredSize(new Dimension(IMG_WIDTH, IMG_HEIGHT));
	...
	private static void launch() throws IOException {
		...
		BufferedImage originalImage = ImageIO.read(new File("src\\ressources\\maze2.jpg"));
		...
		try {
			...
			coordImage = cherchePixels(couleur, mazeBuff);

			// TEST VISUEL : imprimé sur fichier "output.txt"
			// PrintStream out = new PrintStream(new FileOutputStream("output.txt"));
			// System.setOut(out);

			for (Node i : coordImage) {
	 ...
	 */
	private static ArrayList<Node> cherchePixels(Color c, BufferedImage maze) {
	...
	public void paintComponent(Graphics g) {
		...
		//Draws the walls from the picture :
		for(Node n : coordImage) {
			int xBorder = n.getX() - (n.getX() % size);
			int yBorder = n.getY() - (n.getY() % size);
